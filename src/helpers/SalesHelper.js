...
function calculate_item_subtotal(qty, discount, price, vat, includeTax = 1) {
  return new Promise(async (resolve, reject) => {
    ...
    if (includeTax === "1") {
      subtotal =
        subtotal + (await get_item_total_tax_exclusive(qty, price, vat));
    } else {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let invData = {
      ...
      invoice_data: {
        ...
        customer_name: data.customer.length > 0 ? data.customer[0].name : "",
        customer_vat:
          data.customer.length > 0
            ...
            : "",
        customer_address:
          data.customer.length > 0 ? data.customer[0].company_name : "",
        body: itemList,
    ...
    let invData = {
      ...
      invoice_data: {
        invoice_number: `${saleType} ${
          saleType === "WORKORDER" ? data.workorder_id : data.quotation_id
        }`,
