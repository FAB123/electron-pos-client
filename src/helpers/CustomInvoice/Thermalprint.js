class ThermalPrint {
  constructor(invoiceData, storeData) {
    this.invoiceData = invoiceData;
    let { invoice_templates, company_logo } = storeData;
    this.invoice_templates = invoice_templates;
  ...
  get generateprinter() {
    ...
    if (this.invoiceData?.customer_details) {
      if (this.invoiceData?.customer_details?.customer_mobile) {
        customer_info += `<span class="customer_info">${this.invoiceData?.customer_details?.name} (${this.invoiceData?.customer_details?.mobile})</span>
    ...
    let getToken = () => {
      if (this.invoiceData.token) {
        return `<div class="separate-line"></div>
               <span class="company_token">TOKEN :  ${this.invoiceData.token}</span>
        `;
      }
